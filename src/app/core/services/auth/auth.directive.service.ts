...
export class AuthDirectiveService implements DirectiveImplements {
  ...
  switchStatusOfIf(el: any, value: string) {
    ...
    if (this.authService.checkAuth(value)) {
      ...
      if (comment && comment.nodeType === 8 && comment.nodeValue === this.completeDirectiveName) {
  ...
  unmounted(el: any) {
    ...
    if (comment && isCommentElement(comment) && comment.nodeValue === this.completeDirectiveName) {
